/* 
URL shortener -- A google's chromium browser extention intended
to provide easy access to google's URL shortener API.
...
function shortURL(url){
 ...
 xhr.onload = function () {
  ...
  if(!response.match(/error/)){
     response = response.match(/id.+/)[0].match(/http.+/)[0].replace(/[",]+/,"");
     //alert(response);
     googl.value = response;
     googl.focus();
     googl.select();
